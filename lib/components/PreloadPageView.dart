...
class PreloadPageViewWidget extends StatefulWidget {
  ...
  const PreloadPageViewWidget({Key? key, required this.valueNotifier, required this.index, required this.sizingInformation}) : super(key: key);
...
class _PreloadPageViewWidgetState extends State<PreloadPageViewWidget> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      ...
      children: [
        SizedBox(
          height: (widget.sizingInformation.deviceScreenType == DeviceScreenType.desktop) ? MediaQuery.of(context).size.width / 2.5 : MediaQuery.of(context).size.height / 1.5,
          child: PreloadPageView.builder(
              controller: preloadPageController,
              physics: BouncingScrollPhysics(),
              preloadPagesCount: 3,
              itemCount: widget.valueNotifier.value.item2.result[widget.index].posts.length,
              onPageChanged: (page) {
                if (!mounted) return;
                setState(() {
                  widget.valueNotifier.value.item2.result[widget.index].currentPage = page;
                });
              },
              itemBuilder: (context, pageindex) {
                return (widget.valueNotifier.value.item2.result[widget.index].posts[pageindex].toString().contains("mp4") ||
                    widget.valueNotifier.value.item2.result[widget.index].posts[pageindex].toString().contains("mpeg4"))
                    ? GestureDetector(
                  behavior: HitTestBehavior.opaque,
                  onTap: () {
                    Navigator.push(context, MaterialPageRoute(builder: (context) => AssetPageView(stringList: widget.valueNotifier.value.item2.result[widget.index].posts)));
                  },
                  child: Stack(
                    children: [
                      Container(
                ...
                //     })
                    ...
                    child: Image.network(
                      widget.valueNotifier.value.item2.result[widget.index].posts[pageindex],
                      fit: BoxFit.cover,
        ...
        (widget.valueNotifier.value.item2.result[widget.index].posts.length != 0 && widget.valueNotifier.value.item2.result[widget.index].posts.length != 1)
            ? Align(
          alignment: Alignment.center,
          child: Container(
              ...
              children: [
                MaterialButton(
                   ...
                   // color: (themedata.value.index == 0) ? Color(white) : Color(materialBlack),
                    onPressed: () {
                      preloadPageController.previousPage(duration: kDuration, curve: kCurve);
                    },
                    child: Icon(
                ...
                MaterialButton(
                 ...
                 //   color: (themedata.value.index == 0) ? Color(white) : Color(materialBlack),
                    onPressed: () {
                      preloadPageController.nextPage(duration: kDuration, curve: kCurve);
                    },
                    child: Icon(
